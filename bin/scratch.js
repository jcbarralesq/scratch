...
async function main() {
  ...
  program
    ...
    .option('-s, --source <path>', 'Local path or GitHub URL')
    .option('-g, --git <repo>', 'GitHub repository (user/repo or URL)')
    .option('-b, --branch <branch>', 'Git branch', 'main')
    .option('--path <subpath>', 'Subdirectory within repo')
    .action(async (name, options) => {
      const registry = loadRegistry();
      
      // Check if already exists
      if (registry.templates.find(t => t.name === name)) {
        printError(`Template "${name}" already exists`);
        process.exit(1);
      }
      ...
      if (options.git) {
        templateEntry.source = 'github';
        templateEntry.repo = options.git.replace(/^https?:\/\/github\.com\//, '').replace(/\.git$/, '');
        templateEntry.branch = options.branch;
        if (options.path) templateEntry.path = options.path;
      } else if (options.source) {
        if (options.source.startsWith('http') || options.source.startsWith('git@')) {
          templateEntry.source = 'github';
          templateEntry.repo = options.source.replace(/^https?:\/\/github\.com\//, '').replace(/\.git$/, '');
          templateEntry.branch = options.branch;
        } else {
          templateEntry.source = 'local';
          templateEntry.path = path.resolve(options.source);
        }
      } else {
        // Interactive mode
        const answers = await inquirer.prompt([
          ...
          },
          {
            type: 'input',
            name: 'subpath',
            message: 'Subdirectory within repo (optional):',
            when: (answers) => answers.sourceType === 'github'
          }
        ]);
        
        templateEntry.source = answers.sourceType;
        if (answers.sourceType === 'github') {
          templateEntry.repo = answers.repo;
          templateEntry.branch = answers.branch;
          if (answers.subpath) templateEntry.path = answers.subpath;
        } else {
          templateEntry.path = path.resolve(answers.path);
        }
      }
      ...
      printSuccess(`Template "${name}" added to registry`);
    });
  ...
  program
    ...
    .action(async (name, options) => {
      const registry = loadRegistry();
      const index = registry.templates.findIndex(t => t.name === name);
      ...
      printSuccess(`Template "${name}" removed`);
    });
